...
def de_tempo_para_elongamento_vdd(
    taxa_def_vdd: float, tempos: np.ndarray
) -> np.ndarray:
    """Converte de um array de tempos para elongamento, dada uma taxa de deformação de engenharia."""
...
def de_tempo_para_deslocamento(
    taxa_def: float, tempos: np.ndarray, tamanho_inicial=6.0
) -> np.ndarray:
...
def taxa_nominal_com_compliance(
    tempo_total, tamanho_inicial=6.0, deslocamento_real=-1.7
) -> float:
