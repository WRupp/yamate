...
def simulate_vvh_uniaxial_compression(material_properties):
    ...
    strain_rate = cm.taxa_nominal_com_compliance(3000, tamanho_inicial=initial_height, deslocamento_real=-1.7)
    axial_stretches = cm.de_tempo_para_elongamento(taxa_def_eng=strain_rate, tempos=times)
    #axial_displacement = cm.de_tempo_para_deslocamento(taxa_def=strain_rate, tempos=times, tamanho_inicial=initial_height)

    axial_stresses, transversal_stretches = uniaxial.uniaxial_procedure(times, axial_stretches, my_material)
...
def uniaxial_force_l2(design_variables, true_curve, parameters_to_identify, additional_material_parameters):
    ...
    material_properties_trial = {**identifiable_parameters, ** additional_material_parameters}
...
def fit_vvh_to_uniaxial_data():
    ...
    diss_result_forces = pd.read_csv(plga_data_path / "Forcas_resultadosDissertacao.csv")
